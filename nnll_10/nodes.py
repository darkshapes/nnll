...
#             file_extension = os.path.join(folder, file_extension)
...
#         llama_expression.setdefault("n_threads", threads)
#     if max_context is not None:
#         llama_expression.setdefault("n_ctx", max_context)
#     if batch is not None:
#         llama_expression.setdefault("n_batch", batch)
#     if flash_attention is not None:
#         llama_expression.setdefault("flash_attn", flash_attention)
#     return Llama(**llama_expression)
...
# ) -> str:
#     llama_expression = {"messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}]}
#     if streaming:
#         llama_expression.setdefault("stream", streaming)
#     if repeat_penalty:
#         llama_expression.setdefault("repeat_penalty", repeat_penalty)
#     if temperature:
#         llama_expression.setdefault("temperature", temperature)
#     if top_k:
#         llama_expression.setdefault("top_k", top_k)
#     if top_p:
#         llama_expression.setdefault("top_p", top_p)
#     if max_tokens:
#         llama_expression.setdefault("max_tokens", max_tokens)
#     return llama.create_chat_completion(**llama_expression)
...
#     prompt: A[str, Text(multiline=True, dynamic_prompts=True)] = "A slice of a rich and delicious chocolate cake presented on a table in a palace reminiscent of Versailles",
#     negative_terms: A[str, Text(multiline=True, dynamic_prompts=True)] = None,
#     seed: A[int, Dependent(on="prompt"), Numerical(min=0, max=0xFFFFFFFFFFFFFF, step=1, randomizable=True)] = soft_random(),  # cross compatible with ComfyUI and A1111 seeds
#     batch: A[int, Numerical(min=0, max=512, step=1)] = 1,
# ) -> A[list, Name("Queue")]:
#     full_prompt = []
#     full_prompt.append(prompt)
...
#             te_expressions[i]["attn_implementation"] = "flash_attention_2"
...
#     if device is not None:
#         insta.set_device(device)
#     conditioning = {"padding": padding, "truncation": truncation, "return_tensors": return_tensors}
#     queue = insta.encode_prompt(queue, tokenizers_in, text_encoders_in, conditioning)
#     return queue
...
#         if value != None:
#             if value != False:
#                 scheduler_input.setdefault(name, value)
...
#     file_prefix: A[str, Text(multiline=False, dynamic_prompts=True)] = "Shadowbox",
#     device: A[iter, Literal[*spec]] = None,  # type: ignore
#     file_format: A[Literal["png", "jpg", "optimize"], Dependent(on="temp", when=False)] = "png",
#     compress_level: A[int, Slider(min=1, max=9, step=1), Dependent(on="format", when=(not "optimize"))] = 7,
#     temp: bool = False,
# ) -> I[Any]:
#     print("decoding")
