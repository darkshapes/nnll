...
class TallScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        yield Footer()
        yield CentreFoldVertical()

    dimension: reactive[str] = reactive("a")


class CentreFoldHorizontal(Horizontal):  # laptop layout
    def compose(self) -> ComposeResult:
        with Container(id="app-grid-horiziontal"):
            with Container(id="left-frame"):
                yield Static("Vertical layout, child")
            with Container(id="centre-frame"):
                yield Static("This")
                yield Static("panel's")
                yield Static("grid layout!", id="centre-frame-final")
            with Container(id="right-frame"):
                yield Static("Children")
                yield Static("Here")


class CentreFoldVertical(Vertical):  # phone layout
    def compose(self) -> ComposeResult:
        with Container(id="app-grid-vertical"):
            with Container(id="top-frame"):
                yield Static("Vertical layout, child")
            with Container(id="centre-frame"):
                yield Static()
                yield Static("panel's")
                yield Static("grid layout!", id="centre-frame-final")
            with Container(id="bottom-frame"):
                yield Static("Children")
                yield Static("Here")
