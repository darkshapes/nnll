...
from nnll.download.async_download import async_download_session
...
def mock_async_remote_transfer():
    ...
    with mock.patch("nnll.download.async_download.async_remote_transfer", new=mock.AsyncMock()) as mocked:
        yield mocked
...
def mock_async_save_file():
    ...
    with mock.patch("nnll.download.async_download.async_save_file", new=mock.AsyncMock()) as mocked:
        yield mocked
...
def mock_retry():
    ...
    with mock.patch("nnll.download.async_download.retry", new=mock.AsyncMock(side_effect=mock_retry_func)) as mocked:
        yield mocked
...
#     with mock.patch("nnll.download.async_download.async_open", new_callable=mock.AsyncMock) as mock_open:
