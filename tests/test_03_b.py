...
def mock_prepare_download():
    ...
    with mock.patch("nnll_03.prepare_download", new=mock.AsyncMock()) as mocked:
        yield mocked
...
def mock_async_download_session():
    # Ensure async_save_file is correctly mocked as an AsyncMock
    with mock.patch("nnll_03.async_download_session", new=mock.AsyncMock()) as mocked:
        yield mocked
...
def mock_gather_text_lines_from():
    # Ensure async_save_file is correctly mocked as an AsyncMock
    with mock.patch("nnll_03.gather_text_lines_from", new=mock.AsyncMock(return_value=["file1", "file2"])) as mocked:
        yield mocked
...
def mock_gather_empty():
    # Ensure async_save_file is correctly mocked as an AsyncMock
    with mock.patch("nnll_03.gather_text_lines_from", new=mock.AsyncMock(return_value=[])) as mocked:
        yield mocked
...
async def mock_gather_exception():
    # Ensure async_save_file is correctly mocked as an AsyncMock
    with mock.patch("nnll_03.gather_text_lines_from", new=mock.AsyncMock(side_effect=FileNotFoundError("File not found"))) as mocked:
        yield mocked
