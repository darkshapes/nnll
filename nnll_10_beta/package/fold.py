...
from nnll_10_beta.package.display_bar import DisplayBar

from nnll_10_beta.package.message_panel import MessagePanel
from nnll_10_beta.package.response_panel import ResponsePanel
from nnll_10_beta.package.tag_line import TagLine
from nnll_10_beta.package.voice_panel import VoicePanel
...
class Fold(Container):
    ...
    async def pass_text_to_tokenizer(self) -> None:
        ...
        display_bar = self.query_one("#display_bar")
        # display_bar.
        self.query_one("#display_bar").calculate_tokens(self.current_tag, message, self.unit_labels)
    ...
    def _on_mouse_scroll_down(self, event: events.MouseScrollUp) -> None:
        ...
        if self.query_one("#responsive_display").has_focus_within != self.query_one("#response_panel").has_focus:
            ...
            current_tag = tag_line.emulate_scroll_down(tag_line.valid_conversions)
            self.current_tag = str(current_tag).title()
    ...
    def _on_mouse_scroll_up(self, event: events.MouseScrollUp) -> None:
        ...
        if self.query_one("#responsive_display").has_focus_within != self.query_one("#response_panel").has_focus:
            event.prevent_default()
            tag_line = self.query_one("#tag_line")
            current_tag = tag_line.emulate_scroll_up(tag_line.valid_conversions)
            self.current_tag = str(current_tag).title()
