...
def index(folder_name: str = "modules", toc_file: list = ["__init__.py", "README.md"], indicator: str = "src.py"):
...
def main():
    parser = argparse.ArgumentParser(description="Find all [indicator] within [folder], then write [toc] table of contents in [folder].", epilog="Example: nnll-contents src.py modules ['__init__.py', 'README.md']")
    parser.add_argument("-i", "--indicator", nargs="?", help="Filename convention to be indexed", const="src.py", default="src.py")
    parser.add_argument("-f", "--folder", nargs="?", help="Path to ouput and root scan folder (default: os.path.join(os.getcwd(), folder_name 'modules')", const="os.path.join(os.getcwd(), 'modules')", default=os.path.join(os.getcwd(), "modules"))
    parser.add_argument("-t", "--toc", "--contents", help="Output files to create (default: ['__init__.py', 'README.md'])", nargs="*", default=["__init__.py", "README.md"])
