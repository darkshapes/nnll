...
class LayerFilter:
    ...
    def filter_metadata(self, filter_cascade: dict, model_header: dict, tensor_count: int) -> dict:
        ...
        bundle_check = self.handle_values.pull_key_names(filter_cascade["layer_type"], model_header, tensor_count) # Try to find layer type

        if bundle_check is None: # No layer type found
            file_metadata["layer_type"] = "unknown"
            bundle_types = self.handle_values.pull_key_names(filter_cascade["category"], model_header, tensor_count) # Continue anyway
            ...
            if "compvis" != file_metadata["layer_type"]: # only compvis has bundles
                bundle_types = self.handle_values.pull_key_names(filter_cascade["category"], model_header, tensor_count) # Try to find category
                ...
                for category in range(list(filter_cascade["category"].keys())):
                    bundle_types.extend(self.handle_values.pull_key_names(filter_cascade["category"][category], model_header, tensor_count)) # Try to find category
        ...
        if bundle_types is None or bundle_types == []: # If we have no category jump to, search every category
            ...
            for category in categories[2:]:
                file_metadata["model"] = self.handle_values.pull_key_names(filter_cascade[category], model_header, tensor_count)
            ...
            if isinstance(file_metadata["category"], list): # When more than one value has been found in category
                ...
                for category in file_metadata["category"]: # Jump to known values inside
                    if len(bundle_check) >= 1 and len(file_metadata["category"]) > 1: # If we already captured the first model and need more, ignore tensors
                        bundle_check.extend(self.handle_values.pull_key_names(filter_cascade[category], model_header, tensor_count=None))
                    else:
                        bundle_check = (self.handle_values.pull_key_names(filter_cascade[category], model_header, tensor_count)) # First model only
                ...
                try:
                    file_metadata["model"] = self.handle_values.pull_key_names(filter_cascade[file_metadata["category"]], model_header, tensor_count)
